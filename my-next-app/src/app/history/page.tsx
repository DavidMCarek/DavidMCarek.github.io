export default function History() {
  return (
    <main>
      <article>
        <h1>A not so brief history of my software development experience.</h1>

        <p>
          I started out with a strong interest in JavaScript. After watching
          many hours of pluralsight videos I learned most of the quirks of
          JavaScript and how to develop in a clean maintainable way. At the time
          I was using mostly plain old JavaScript with some jQuery as well. I
          also had a bit of a catch phrase.
        </p>
        <blockquote>
          JavaScript is like the wild west so it's easy to tell the difference
          between a good JavaScript developer and a bad one.
        </blockquote>
        <p>
          This held true in my experiences especially because I was working with
          a lot of co-ops whose experience with software development focused on
          strongly typed object oriented languages that have guardrails in place
          due to how opinionated they are.
        </p>
        <p>
          As I continued in my career I dabbled in the major FE frameworks.
          While
        </p>
      </article>
    </main>
  )
}
